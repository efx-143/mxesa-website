...
test('reads the two Fest formats out of a name', () => {
  assert.equal(festFormatFromName('MXESA Berlin Hack Day'), 'hackDay');
  assert.equal(festFormatFromName('MXESA London Meet Up'), 'meetUp');
});

/* The shape MLH actually uses: format immediately after "MXESA",
   then the city, then a partner after an "x". */
test('reads the real name shape, partner and all', () => {
  assert.equal(
    festFormatFromName('MXESA Meet Up Toronto x Hack the 6ix'),
    'meetUp',
  );
  assert.equal(festFormatFromName('MXESA Hack Day New York x IBM'), 'hackDay');
});
...
test("the name's own format outranks a partner that mentions the other", () => {
  assert.equal(
    festFormatFromName('MXESA Meet Up Berlin x Hack Day Collective'),
    'meetUp',
  );
  assert.equal(
    festFormatFromName('MXESA Hack Day Lagos x The Meet Up Co'),
    'hackDay',
  );
});
...
test('is case-insensitive and tolerates the usual spellings', () => {
  ['Hack Day', 'hack day', 'HACK DAY', 'Hackday', 'Hack-Day'].forEach(
    (spelling) => {
      assert.equal(
        festFormatFromName(`MXESA Lagos ${spelling}`),
        'hackDay',
        spelling,
      );
    },
  ...
  ['Meet Up', 'meet up', 'Meetup', 'meet-up', 'MEETUP'].forEach((spelling) => {
    assert.equal(
      festFormatFromName(`MXESA Lima ${spelling}`),
      'meetUp',
      spelling,
    );
  });
...
/* The live payload carries "Quinn's non-hack day mxesa test". A
...
test('"non-hack day" is not a Hack Day', () => {
  assert.equal(festFormatFromName("Quinn's non-hack day mxesa test"), null);
  assert.equal(festFormatFromName('MXESA Non Hack Day Sydney'), null);
  assert.equal(festFormatFromName('Non-Hack Day MXESA Toronto'), null);
});
...
test('an on-convention name is not confused by a second format later', () => {
  assert.equal(festFormatFromName('MXESA Hack Day and Meet Up'), 'hackDay');
});
...
test('a name with neither format returns null', () => {
  assert.equal(festFormatFromName('Stephen Test MXESA'), null);
  assert.equal(festFormatFromName('SharkHacks3 at MXESA'), null);
  assert.equal(festFormatFromName('Hacktober Fest Manila'), null);
});

/* "hacks day" is not "hack day", and "MXESA" is not either. The
...
test('near-misses do not match', () => {
  ...
  assert.equal(festFormatFromName('MXESA'), null);
