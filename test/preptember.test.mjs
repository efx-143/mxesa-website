...
/* One greeting on both sides of the Preptember flag: "your
   MXESA", never a month name. The preptemberAccent swap retired
   2026-08-18 — its reappearance would mean the hero names the wrong
   thing again for one side of the flag. */
test('the welcome accent says MXESA and never swaps', () => {
  assert.match(my.welcome.accent, /MXESA/);
...
test('the open resource URLs are real and the brand kit is a placeholder', () => {
  assert.equal(
    HOST_HANDBOOK_URL,
    'https://mlh.gitbook.io/mlh-mxesa-organizer-guide',
  );
  ...
  assert.equal(HOST_EMAIL_URL, 'mailto:mxesa@mlh.io');
...
test('the /my apply link points at the MLH application and carries its tag', () => {
  ...
  assert.equal(mine.pathname, '/host/mxesa-2026');
