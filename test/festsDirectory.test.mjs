...
/* The live payload's shape, not a hypothetical one: the API sends an ISO
   ...
   "hackathon" for every MXESA event there is. */
...
test('splits the hosting partner out of the event name', async (t) => {
  withFetch(t, () =>
    jsonResponse({
      events: [{ ...EVENT, name: 'MXESA Meet Up Toronto x Hack the 6ix' }],
      count: 1,
    }),
  );
  ...
  assert.equal(fest.name, 'MXESA Meet Up Toronto');
